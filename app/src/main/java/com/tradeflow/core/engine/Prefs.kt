 ...
 * Patch #2: synchronous commit() everywhere (data is tiny, ~ms).
 * Saves are now physically un-losable even if the process dies right after.
 */
object Prefs {
    ...
    fun setBusy(ctx: Context, b: Boolean) { sp(ctx).edit().putBoolean("busy", b).commit() }
    ...
    fun setBiz(ctx: Context, name: String, phone: String) {
        sp(ctx).edit().putString("biz_name", name).putString("biz_phone", phone).commit()
    }
    ...
    fun setServices(ctx: Context, list: List<String>) {
        sp(ctx).edit().putString("services_csv", list.joinToString("|")).commit()
    }
    ...
    fun setWork(ctx: Context, days: Set<Int>, startH: Int, endH: Int) {
        sp(ctx).edit().putString("work_days", days.sorted().joinToString(","))
            .putInt("work_start", startH).putInt("work_end", endH).commit()
    }

    fun typicalJobMins(ctx: Context): Int = sp(ctx).getInt("typical_mins", 120)
    fun setTypicalJobMins(ctx: Context, m: Int) { sp(ctx).edit().putInt("typical_mins", m).commit() }
    ...
    fun setLinks(ctx: Context, pay: String, review: String) {
        sp(ctx).edit().putString("pay_link", pay).putString("review_link", review).commit()
    }

    fun autoConfirm(ctx: Context): Boolean = sp(ctx).getBoolean("auto_confirm", false)
    fun setAutoConfirm(ctx: Context, b: Boolean) { sp(ctx).edit().putBoolean("auto_confirm", b).commit() }

    // ---------- call-tracking scratch ----------
    fun setLastRinging(ctx: Context, num: String?, at: Long) {
        sp(ctx).edit().putString("ring_num", num).putLong("ring_at", at).commit()
    }
    ...
    fun clearLastRinging(ctx: Context) {
        sp(ctx).edit().remove("ring_num").remove("ring_at").commit()
    }

    fun lastHandledCallDate(ctx: Context): Long = sp(ctx).getLong("handled_call_date", 0)
    fun setLastHandledCallDate(ctx: Context, d: Long) {
        sp(ctx).edit().putLong("handled_call_date", d).commit()
    }

    // ---------- alert throttles ----------
    fun lastPermNag(ctx: Context): Long = sp(ctx).getLong("perm_nag", 0)
    fun setLastPermNag(ctx: Context, t: Long) { sp(ctx).edit().putLong("perm_nag", t).commit() }
    fun lastFailAt(ctx: Context): Long = sp(ctx).getLong("fail_at", 0)
    fun setLastFailAt(ctx: Context, t: Long) { sp(ctx).edit().putLong("fail_at", t).commit() }
